...
const SnakeGameCore = ({ onScoreUpdate, onGameStateChange }) => {
  ...
  }, [snake, food, direction, score, gameRunning, gameOver]);

  // Load game state
  const loadGameState = useCallback((state) => {
    setSnake(state.snake || INITIAL_SNAKE);
    setFood(state.food || INITIAL_FOOD);
    setDirection(state.direction || INITIAL_DIRECTION);
    setScore(state.score || 0);
    setGameRunning(state.gameRunning || false);
  ...
  }, [snake, food, direction, score, gameRunning, gameOver, onGameStateChange, getGameState]);
    ...
    <div className="flex flex-col items-center">
      <div className="nokia-screen bg-gray-800 p-6 rounded-lg border-4 border-gray-600 shadow-2xl">
        ...
        <div className="text-center mb-4">
          <h1 className="text-2xl font-bold text-green-400 font-mono">🐍 SNAKE</h1>
        ...
        <div className="mt-4 text-center">
          <div className="grid grid-cols-3 gap-2 max-w-48 mx-auto">
            ...
            <button 
              ...
              onClick={() => !gameRunning && !gameOver ? startGame() : setDirection(direction.y === 0 ? { x: 0, y: -1 } : direction)}
            >
              ↑
            </button>
            <div></div>
            <button 
              className="nokia-btn text-xs"
              onClick={() => gameRunning ? setDirection(direction.x === 0 ? { x: -1, y: 0 } : direction) : null}
            >
              ←
            </button>
            <button 
            ...
            <button 
              className="nokia-btn text-xs"
              onClick={() => gameRunning ? setDirection(direction.y === 0 ? { x: 0, y: 1 } : direction) : null}
            >
              ↓
            </button>
            <div></div>
